%%%----------------------------------------------------------------------
%%% File    : node_flat_static_options_sql.erl
...
-module(node_flat_static_options_sql).
...
get_entity_affiliations(Host, Owner) ->
    ...
    {result,
	 ...
	 {selected, RItems} ->
	     [{mod_pubsub:tree_action(Host, raw_to_node, [Host, {N, <<"">>, T, I}]),
...
get_entity_subscriptions(Host, Owner) ->
    ...
    {result,
	 ...
	 {selected, RItems} ->
	     lists:foldl(
	       fun({N, T, I, J, S}, Acc) ->
		       Node = mod_pubsub:tree_action(Host, raw_to_node, [Host, {N, <<"">>, T, I}]),
		       Jid = decode_jid(J),
		       lists:foldl(
			 fun({Sub, SubId}, Acc2) ->
