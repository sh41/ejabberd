%%%----------------------------------------------------------------------
%%% File    : nodetree_tree_static_options_sql.erl
...
-module(nodetree_tree_static_options_sql).
...
%%	    []
%%    end,
    Options = mod_pubsub:config(Host, default_node_config, []),
	  ?DEBUG("STATIC OPTIONS raw_to_node, ~p, ~p, ~p, ~p, ~p, ~p", [Host, Node, Parent, Type, Nidx, Options]),
